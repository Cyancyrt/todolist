  ...
  <form id="activity-form" class="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-6 sm:p-8 space-y-6 border border-gray-100"
    ...
    <div class="text-center space-y-2">
      <div class="flex items-center justify-center space-x-2">
        <svg class="w-8 h-8 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Aktivitas</h2>
      </div>
      <p class="text-gray-600 text-sm sm:text-base">Perbarui detail aktivitas Anda.</p>
    </div>
    ...
    <div class="space-y-4">
      ...
      <label class="block">
        ...
        <input type="text" name="name" placeholder="Contoh: Belajar JavaScript" 
               ...
               required minlength="3" maxlength="150" value="<?= esc($activity['name']) ?>">
      ...
      <label class="block">
        ...
        <select name="type" class="mt-1 block w-full border border-gray-300 rounded-lg p-4 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all duration-200 hover:shadow-md">
          <option value="personal" <?= $activity['type'] == 'personal' ? 'selected' : '' ?>>Personal (untuk diri sendiri)</option>
          <option value="social" <?= $activity['type'] == 'social' ? 'selected' : '' ?>>Social (bersama orang lain)</option>
        </select>
      </label>

      <input type="hidden" name="created_by" value="<?= esc($activity['created_by']) ?>">
      ...
      <label class="block">
        ...
        <textarea name="description" placeholder="Ceritakan sedikit tentang aktivitas ini..."
                  ...
                  maxlength="500" rows="3"><?= esc($activity['description']) ?></textarea>
      </label>
    </div>
...
<?php 
    ...
    if (!empty($schedules)) {
        foreach($schedules as $sch) {
            // Format Y-m-d H:i agar sesuai dengan Flatpickr dateFormat
            $savedDates[] = date('Y-m-d H:i', strtotime($sch['next_run_at']));
        }
    }
?>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 1. Data dari PHP
    const preselectedDates = <?= json_encode($savedDates) ?>;

    // 2. Hapus Alert Otomatis
    setTimeout(() => {
    ...
    const fp = flatpickr("#schedule-dates", {
        ...
        time_24hr: true,
        disableMobile: "true",
        defaultDate: preselectedDates, // Pre-select dates
        minDate: "today", // FITUR: Mencegah pilih tanggal lampau di kalender
    ...
    // Render chips awal saat halaman dimuat
    ...
    // ============================================================
    // VALIDASI TANGGAL (Termasuk Cek Waktu Lampau)
    // ============================================================
    ...
    // 4. Validasi Real-time Input Biasa
