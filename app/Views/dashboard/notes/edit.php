...
<main class="flex-1 min-h-screen flex items-center justify-center p-4 sm:p-6 bg-gradient-to-br from-blue-50 to-purple-50">
  <div class="w-full max-w-3xl">
    ...
    <form id="note-form" class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 space-y-6 border border-gray-100"
          action="<?= base_url('dashboard/notes/update/' . $task['id']) ?>" method="POST" novalidate>
      <?php csrf_field() ?>
      ...
      <div class="text-center space-y-2">
        <div class="flex items-center justify-center space-x-2">
          ...
          <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Catatan</h2>
        </div>
        <p class="text-gray-600 text-sm sm:text-base">Perbarui catatan aktivitas Anda.</p>
      </div>
        ...
        <div class="grid grid-cols-1 gap-4">
          <label class="block">
            ...
            <input type="text" name="title" placeholder="Contoh: Ide proyek baru atau Renungan harian"
                   ...
                   required minlength="1" maxlength="150" value="<?= esc(old('title', $task['title'] ?? '')) ?>">
          ...
          <label class="block">
            ...
            <textarea 
                name="content" 
                placeholder="Tulis detail catatan Anda di sini... bisa berupa ide, catatan harian, atau apa saja!"
                ...
                maxlength="10000" 
                rows="5"
            ><?= esc(old('content', $task['content'] ?? '')) ?></textarea>
      ...
      <?php if (isset($activities) && is_array($activities) && count($activities) > 0): ?>
        <?php
        $selectedId = old('activity_id') ?? ($task['activity_id'] ?? '');
        $selectedName = '';
        ...
        <div class="space-y-4">
          ...
          <div 
            x-data="{
            open: false,
            selectedText: '<?= esc($selectedName ?: 'Pilih aktivitas (opsional)') ?>',
            selectedValue: '<?= esc($selectedId) ?>'
            }" 
            class="relative"
          >
            <button type="button"
              ...
              <svg :class='{ "rotate-180": open }' class="w-5 h-5 text-gray-500 transform transition-transform duration-200"
